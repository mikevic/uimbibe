<?php
/*
	Player Name: JW Player 5.6 with skin
	Description: JW Player 5.6 with skin
	Author: Pandu
	ClipBucket Version: 2
	Plugin Version: 1.0 - JW 5
	Website: http://youseetube.com/
*/
...
if(!function_exists("jwskins"))
{
	function jwskins($data)
	{
		...
		if($vid_file || $is_youtube)
		{
			...
			$swfobj->playerFile = PLAYER_URL.'/jwskins/player-viral.swf';
			...
			if($cb_jw_smart)
			{
				...
				//Setting up the logo
				$swfobj->addVar('skin',PLAYER_URL.'/jwskins/stormtrooper.zip');            
                ...
                $swfobj->addVar('plugins','viral-2,ltas');
                $swfobj->addVar('viral.onpause','true');
				              
                $swfobj->addVar('ltas.cc','ccedcrkplzmwvrc');
	...
	register_actions_play_video('jwskins');
